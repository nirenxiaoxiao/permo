...
LRESULT CTextProgressCtrl::OnSetText(WPARAM wp, LPARAM sText)
{
	LPCTSTR szText = (LPCTSTR)sText;
...
{
    if (!_tcsncpy((TCHAR*)szText, m_strText, cchTextMax))
        return 0;
    else 
...
void CTextProgressCtrl::ShowMyText(CString text, bool show)
{
	m_showMyText=show;
	if (!text.IsEmpty())
	{
...
void CTextProgressCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	//SetCapture();
	CTextProgressCtrl::OnRButtonUp(nFlags, point);
}

void CTextProgressCtrl::SetFontSize(int nFontSize)
{
